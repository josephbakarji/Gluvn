...
int mpu_init(struct int_param_s *int_param)
{
    ...
    if (rev) {
...
#ifdef MPU_DEBUG
            mySerial.print("Unsupported software product rev: "); mySerial.println(rev);
#endif
            return -4;
        }
    } else {
        ...
        if (!rev) {
#ifdef MPU_DEBUG
            mySerial.println("Product ID read as 0 indicates device is either incompatible or an MPU3050");
#endif
            return -6;
        } else if (rev == 4) {
#ifdef MPU_DEBUG
            mySerial.println("Half sensitivity part found.");
#endif
            st->chip_cfg.accel_half = 1;
        } else
            st->chip_cfg.accel_half = 0;
...
#ifdef AK89xx_SECONDARY
#ifdef MPU_DEBUG
    mySerial.println("Setting up compass");
#endif
    errCode = setup_compass();
    if (errCode != 0) {
#ifdef MPU_DEBUG
       mySerial.print("Setup compass failed: "); mySerial.println(errCode); 
#endif
    }
...
#ifdef MPU_DEBUG
            mySerial.print("fimrware write failed: ");
            mySerial.println(errCode);
#endif
            return -3;
        }
...
#ifdef MPU_DEBUG
            mySerial.print("Firmware compare failed addr "); mySerial.println(ii);
            for (int i = 0; i < 10; i++) {
              mySerial.print(progBuffer[i]); 
              mySerial.print(" ");
            }
            mySerial.println();
            for (int i = 0; i < 10; i++) {
              mySerial.print(cur[i]); 
              mySerial.print(" ");
            }
            mySerial.println();
#endif
...
#ifdef MPU_DEBUG
    mySerial.println("Firmware loaded");
#endif
    return 0;
}
...
#ifdef MPU_DEBUG
        mySerial.println("Compass not found.");
#endif
        return -1;
    }
...
#ifdef MPU_DEBUG
    mySerial.print("Compass sens: "); mySerial.print(st->chip_cfg.mag_sens_adj[0]); mySerial.print(" ");
    mySerial.print(st->chip_cfg.mag_sens_adj[1]); mySerial.print(" ");
    mySerial.print(st->chip_cfg.mag_sens_adj[2]); mySerial.println();
#endif
